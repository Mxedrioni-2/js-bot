...
class MusicCog(commands.Cog):
    ...
    async def resolve_url(self, original_url):
        try:
    ...
    async def play_next(self, ctx):
        ...
        if loop_state == 1 and guild_id in self.current_song:
            ...
            resolved = await self.resolve_url(next_song['original_url'])
            if not resolved:
            ...
            resolved = await self.resolve_url(next_song['original_url'])
            if not resolved:
                await ctx.send(f"სიმღერა ვერ ჩაიტვირთა, ვაგრძელებ...")
                if queue:
    ...
    async def process_url(self, url, download=False):
        ...
        quick_config['extract_flat'] = 'in_playlist'
        try:
            ...
            with yt_dlp.YoutubeDL(quick_config) as ydl:
                ...
                if info['entries'] and not is_playing:
                    ...
                    if first_entry:
                        try:
                            first_song = {
                                'original_url': first_entry.get('webpage_url') or first_entry.get('url'),
                                'title': first_entry.get('title', 'First Track')
                            }
